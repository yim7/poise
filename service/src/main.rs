    ...
    let router = if let Some(config) = loaded_config.as_ref() {
        ...
        let application = match (env::var("GRID_PLATFORM_SERVICE_DB_PATH"), binance_config) {
            (Ok(path), Some(config)) => {
                ...
                Application::bootstrap_with_sqlite_and_binance(path, config, Arc::new(transport))
            ...
            (Err(_), Some(config)) => {
                let transport = RealBinanceTransport::new(&config)
                    .with_context(|| "failed to build real binance transport")?;
                Application::bootstrap_with_binance(config, Arc::new(transport))
            }
...
fn registry_from_service_config(config: &ServiceConfig) -> anyhow::Result<ApplicationRegistry> {
    let binance = binance_registry_options_from_env();
    let instances = config
        .instances
        .iter()
        .map(|instance| bootstrap_instance_application(config, instance, binance.as_ref()))
...
fn bootstrap_instance_application(
