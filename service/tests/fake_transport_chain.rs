...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn fake_transport_trade_only_event_uses_latest_trade_price_for_paper_fill() -> Result<()> {
    let now_ms = Utc::now().timestamp_millis();
    let transport = ScriptedBinanceTransport::new(
        ExchangeSymbol {
            symbol: "XAUUSDT".into(),
    ...
    runtime.snapshot = paused_snapshot_with_buy_order();
    runtime.snapshot.runtime.env = "testnet".into();
    runtime.snapshot.runtime.mark_price = 120.0;
    ...
    market_tx
        ...
        .await
        .context("failed to send scripted trade-only event")?;
    ...
    .await
    .context("trade-only event did not drive paper fill")?;
