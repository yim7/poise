...
| Task | 状态 | 验收 | Commit |
| --- | --- | --- | --- |
| 1.1 定义健康状态模型 | pending | 能表达 market data、user data、effect worker、recovery、account monitor、PNL backfill 的状态 |  |
| 1.2 增加启动前 preflight | pending | 启动前校验账户模式、保证金模式、symbol metadata、mark price、available balance、最小交易单位 |  |
| 1.3 暴露健康查询接口 | pending | HTTP 能返回整体状态、每个 task 最近成功时间、最近错误摘要 |  |
...
| Task | 状态 | 验收 | Commit |
| --- | --- | --- | --- |
| 3.1 增加配置解释模型 | pending | 对每个 track 展示 unit 对应 native quantity、USD 面值、最小步长、最大名义、loss limit 资产 |  |
...
| Task | 状态 | 验收 | Commit |
| --- | --- | --- | --- |
| 4.1 定义 replay 输入格式 | pending | 支持读取价格序列、初始仓位、手续费率、当前 track 配置 |  |
| 4.2 输出仓位和交易统计 | pending | 输出最大 contracts、最大 USD 面值、估算 BTC 手续费、成交次数、区间内仓位分布 |  |
| 4.3 对比参数组合 | pending | 能对比 `min_rebalance_units`、杠杆、notional per unit、区间宽度的影响 |  |
...
| Task | 状态 | 验收 | Commit |
| --- | --- | --- | --- |
| 5.1 更新系统概览 | pending | PNL、数量单位、OKX 运行语义与当前实现一致 |  |
