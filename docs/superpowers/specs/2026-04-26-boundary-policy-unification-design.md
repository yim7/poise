...
- 不在同一批里重做 read model enum 投影。
- 不改变 ManualOverride / Flatten 的外层优先级。
...
- `policy.rs` 拥有 Normal 模式下的 boundary 覆盖决策：
  ...
  - 判断 active maker 是否仍可覆盖。
  - 判断 maker 是否已经过 grace。
  - 产出 desired binding intents。
- `planning.rs` 只做编排：
  - 建立 ledger view。
  - 应用全局 gate。
  - 调用 policy。
  - reconcile desired binding 与 existing binding。
...
pub struct BoundaryPolicyInput<'a> {
    ...
    pub existing_bindings: &'a [LiveOrderBinding],
    pub submit_intent: &'a SubmitIntentInput<'a>,
    pub observed_at: DateTime<Utc>,
}

pub struct BoundaryPolicyOutput {
    pub desired_bindings: Vec<DesiredBinding>,
    pub binding_updates: Vec<BindingLifecycleUpdate>,
}

pub fn plan_boundary_policy(input: BoundaryPolicyInput<'_>) -> BoundaryPolicyOutput;
```

这里的 `DesiredBinding` 仍然可以区分 passive maker 与 aggressive catch-up，但这是 policy 输出的执行形态，不再是调用层串行跑两个 policy 的协议。
...
- 如果 active maker 过 grace，本轮 policy 可以产出 aggressive desired binding，并要求 reconciliation 取消旧 maker owner。
...
- ManualOverride > Flatten > Normal 的外层优先级不变。
...
- 先补行为测试，再改结构。
- 重构后删除 `CoverageReservation`、`effective_maker_owner_indexes`、`replaceable_active_owner_indexes`。
