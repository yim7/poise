...
- `planning.rs` 只做编排：
  ...
  - 应用全局 gate。
  - 调用 policy。
  - 根据 policy 输出构造 desired binding。
  - reconcile desired binding 与 existing binding。
  - 在 reconciliation 私有逻辑里判断 active maker 是否仍可覆盖、maker grace 是否已过期。
...
`BoundaryPolicy` 输出 operation selection，而不是直接生成订单。这样 `policy.rs` 拥有 Normal 模式的覆盖选择，`planning.rs` 仍然拥有定价、数量、min-notional 过滤和 effect 构造。
