...
这里的检查是账本不变量，不是展示修正。实现不能把越界值静默 clamp 成正常视图；一旦发现重复成交、错误回写或超量吸收导致越界，planner / recovery 必须进入 `BoundaryProgressOutOfRange` anomaly，停止继续基于被修正过的假视图规划。
...
`BoundaryLedgerState` / `ExecutorState` 是内存运行态，不实现持久化 serde，也不提供通用 `to_document()` 投影。需要在当前进程内做 mutation 回滚时，engine 捕获 `TrackMutationFrame`；这个 frame 只服务 rollback，不是重启恢复协议，也不是读取层输入。若某个字段确实需要跨 JSON/DB 边界，应为那个持久化边界定义自己的 serde DTO；executor runtime state 不作为持久化协议。
...
- `View`：engine 提供的当前进程只读查询结果，例如 `TrackRuntimeView`、`ExecutorView`、`BindingView`、`TrackLiveView`。`View` 可以由 runtime 内部状态派生，但不能暴露 `progress`、cancel-pending 或 rollback 专用状态。
- `ReadSource`：application 组装 read model 前的输入聚合，例如 `TrackReadSource`。它可以组合 track definition、engine `View`、recent events、effect journal 等不同来源。
- `ReadModel`：API / UI 使用的最终展示 DTO，例如 `TrackReadModel`、`TrackListReadModel`。

`Projection` 不作为结构体后缀使用，只保留在 `project_*` 这类函数名里，表示从 source/model 派生展示字段。当前进程内 mutation rollback 使用 `Frame` 命名，例如 `TrackMutationFrame`；不要再用 `Snapshot` 表示 executor 恢复协议或读取协议。

### 7.7 `expected_exposure`
...
pub struct SubmitRecoveryToken(/* opaque */);
```
...
- `absorbed_exposure_qty` 表示这张 binding 已经被吸收到 boundary progress 的 exposure 数量
- `order_id` 是交易所 live order id；提交前可以为空
- `policy_state` 只保存 owner policy 私有的运行态，不进入通用 binding 语义
- 边界进度由 fills 吸收后再统一回写
- `SubmitOrder` effect 若需要为恢复保留额外上下文，只能携带 executor 私有的 `SubmitRecoveryToken`，不能把 `proposal_key / allocations / policy_state` 直接暴露到 protocol / persistence 边界
...
1. 先把交易所回报的累计成交量吸收到 binding 上，更新 `absorbed_exposure_qty` 与订单生命周期状态
2. 再把本次新增吸收的 exposure 增量按顺序回写到 boundary progress

当一个 binding 覆盖多条边界操作时，部分成交按 **操作顺序贪心分配**，不按比例分配。

交易所回报必须提供“当前累计成交量”视图，而不是让 recording 通过 `PartiallyFilled/Filled/Canceled` 这些生命周期状态猜测成交进度。recording 的规则是：

- `PartiallyFilled`：吸收新增累计成交量，binding 保持 working
- `Canceled / Expired / Rejected`：吸收回报中已经发生的累计成交量，然后终止 binding，不回滚 progress
- `Filled`：吸收到完整成交量；如果提交回报没有带成交量但状态已经是 `Filled`，按整张订单完成处理
- 重复回报：如果累计成交量没有增加，不重复写 boundary progress
...
4. proposal 无匹配 active binding，且对应操作当前没有别的 active owner → `SubmitOrder`
...
7. 外部接口继续保持：
   ...
   - 其中 `ExecutionAction::SubmitOrder` 的恢复信息保持 opaque token，不暴露 boundary / binding 内部结构
...
- 不继续对外保留没有稳定语义来源的旧 `execution_stats / gap_age_ms` 读模型字段
