...
当前 `desired_exposure` 在系统里混合了两种语义：
...
- 执行器新增 `active_round`
- 当前执行计划围绕 `active_round.desired_exposure` 和单个 `inventory_core` 槽位运行
- `mode` 变成 round 级策略
...
它替代当前外层运行态里的 `desired_exposure` 主语义。
...
第一阶段对外协议仍保留字段名 `desired_exposure`，但这里明确：

- 协议层 `desired_exposure` 只投影 `desired_exposure`
- 它不得表示 `ExecutionRound.desired_exposure`
...
### 5.3 `ExecutionRound.desired_exposure`
...
- `ExecutionRound.desired_exposure` 只有在开启新 round 或切换 round 时才变化

第一阶段明确：

- `ExecutionRound.desired_exposure` 是执行层 target 的唯一 owner
- 当前 `WorkingOrder.desired_exposure` 不再保留
- slot 或 order 若需要展示或判断当前 round target，只能读取 `active_round.desired_exposure`
...
- `WorkingOrder` 只保留交易所订单事实和执行角色，不再持有 `desired_exposure`
...
- 等 `active_round` 落地后，必须在同一个 task 内把执行 target 一次性切到 `active_round.desired_exposure`
...
10. 协议层 `desired_exposure` 只投影 `desired_exposure`，不得表示 `ExecutionRound.desired_exposure`
...
1. `desired_exposure` 可以继续随新价格更新，但不会自动改写 `active_round.desired_exposure`
