...
- `server/src/config.rs`：显式解析 `grid_id` 和 `venue`
...
- `configs/test.toml`：更新显式 `grid_id` 与 `venue`
- `docs/protocol-contract.md`：示例中不再假设 `grid_id == symbol`
...
- [x] **Step 1: 先写失败测试，锁住显式 `grid_id` 和市场绑定约束**

```rust
#[test]
fn parses_config_with_explicit_grid_id_and_venue() {
...
#[test]
fn resolve_grid_id_returns_registered_grid_id() {
    ...
    assert_eq!(
        manager.resolve_grid_id(&Instrument::new(Venue::Binance, "BTCUSDT")),
...
```bash
cargo test -p grid-server config::tests::parses_config_with_explicit_grid_id_and_venue
cargo test -p grid-server assembly::tests::assemble_rejects_duplicate_instruments
cargo test -p grid-engine manager::tests::resolve_grid_id_returns_registered_grid_id
```
...
- [x] **Step 5: 配置层改成显式 `grid_id` 与 `venue`**

要求：

- `server/src/config.rs` 中的 `GridDefinition` 解析新字段
- 不再保留 `grid_id()` 由 `symbol` 派生的逻辑
- `assemble()` 同时校验 `grid_id` 唯一和 `Instrument` 唯一
...
```bash
cargo test -p grid-engine manager::tests::
cargo test -p grid-server config::tests:: assembly::tests::
```
...
async fn save_and_load_grid_runtime_snapshot_roundtrip() {
    ...
    storage.save_transition(&snapshot.grid_id, &snapshot, &[]).await.unwrap();

    let loaded = storage.load_grid_state(&snapshot.grid_id).await.unwrap().unwrap();
...
async fn observe_persists_engine_snapshot_without_server_side_snapshot_builder() {
    ...
    assert_eq!(transition.snapshot.grid_id.as_str(), "btc-core");
}
```
...
```bash
cargo test -p grid-engine manager::tests::snapshot_roundtrip_preserves_runtime_state
cargo test -p grid-storage sqlite::tests::save_and_load_grid_runtime_snapshot_roundtrip
cargo test -p grid-server application::tests::observe_persists_engine_snapshot_without_server_side_snapshot_builder
```
...
```bash
cargo test -p grid-engine manager::tests::
cargo test -p grid-storage sqlite::tests::
...
```bash
cargo test -p grid-engine manager::tests::observe_market_reconciles_and_returns_effects
cargo test -p grid-engine manager::tests::command_reconcile_uses_cached_reference_price
cargo test -p grid-server runtime::tests::position_update_reconciles_via_command_without_faking_market_tick
```
...
```bash
cargo test -p grid-engine manager::tests:: reconciler::tests::
cargo test -p grid-server runtime::tests::
```
...
```bash
cargo test -p grid-server http::tests::submit_command_uses_service_command_api
cargo test -p grid-server runtime::tests::startup_sync_observes_exchange_state_before_subscribing
cargo test -p grid-server websocket::tests::broadcasts_events_from_persisted_transition
```
...
- [x] **Step 5: 用显式非 symbol 的 `grid_id` 跑通服务端集成测试**

要求：

- 测试用例不要再用 `grid_id == symbol`
- 至少有一个真实路径用 `grid_id = "btc-core"`、`symbol = "BTCUSDT"`
...
```bash
cargo test -p grid-server http::tests:: runtime::tests:: websocket::tests:: tests::
```
...
- [x] **Step 1: 先写失败测试，锁住 `grid_id` 不再等于 `symbol`**
...
```bash
cargo test -p grid-tui protocol::tests::deserializes_grid_snapshot_with_non_symbol_grid_id
```
...
- `README.md` 示例配置改成显式 `grid_id` 和 `venue`
- `docs/protocol-contract.md` 的响应样例展示非 symbol 的 `grid_id`
- TUI fixture 使用 `btc-core` 一类显式 id，保留 `symbol = BTCUSDT`
- 保证用户能从文档中看出 `grid_id` 与 `symbol` 的职责不同
