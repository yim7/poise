...
> **2026-04-22 修订说明：** 本版计划已经放弃旧的 `trigger_bps / price_confirm` 目标形状，改为 `trigger / reentry_confirm`。以下 task 以当前修订版为准，之前围绕旧形状的执行记录不再作为当前验收依据。

**Goal:** 让 track 带外保护语义和状态机与最新设计一致：

- `freeze`：出主带即冻结，不补仓，回主带立即恢复
- `flatten`：按 `trigger` 决定何时真正平到 `0`，之后按恢复规则自动恢复
- `terminate`：出主带直接终止
- `hold` 删除
...
- 当前推荐配置语法继续使用 inline table，不把子表作为第一版目标
...
- 对外行为必须已经正确：
  ...
  - `flatten.trigger = "immediate"` 出主带直接进入 `flattening`
  - `flatten.trigger = { flatten_confirm = ... }` 出主带先表现为 `frozen`
...
- `cargo test -p poise-core strategy::tests::band_protection_policy_parses_flatten_with_immediate_trigger_and_back_in_band -- --exact`
- `cargo test -p poise-core strategy::tests::band_protection_policy_parses_flatten_with_flatten_confirm_and_reentry_confirm -- --exact`
- `cargo test -p poise-server config::tests::config_toml_parses_flatten_trigger_and_reentry_confirm_policy -- --exact`
- `cargo test -p poise-server projector::tests::projector_shows_flatten_trigger_and_recover_policy -- --exact`
- `cargo test -p poise-engine reconciler::tests::flatten_policy_immediate_trigger_enters_flattening_with_current_runtime_shape -- --exact`
- `cargo test -p poise-engine reconciler::tests::flatten_policy_uses_flatten_confirm_before_flattening_with_current_runtime_shape -- --exact`
- `cargo test -p poise-tui views::instance::tests::renders_flatten_trigger_policy_name -- --exact`
- `cargo test -p poise-track-tuning-workbench commands::tests::load_config_file_preserves_non_default_flatten_policy_when_exporting_track -- --exact`
...
- 删除：
  - `Holding`
  - 通用 `ReentryGuard`
- `flatten.trigger = "immediate"` 必须直接进入 `Flattening`，不允许经过 `FlattenPending`
- `FlattenPending` 只允许用于 `flatten.trigger = { flatten_confirm = ... }`
...
- `cargo test -p poise-engine reconciler::tests::flatten_policy_immediate_trigger_bypasses_flatten_pending -- --exact`
- `cargo test -p poise-engine reconciler::tests::flatten_policy_uses_flatten_pending_before_flattening_when_trigger_is_flatten_confirm -- --exact`
...
- `cargo test -p poise-core strategy::tests::band_protection_policy_parses_flatten_with_flatten_confirm_and_reentry_confirm -- --exact`
- `cargo test -p poise-engine reconciler::tests::flatten_pending_rearms_when_price_flips_to_opposite_out_of_band_side -- --exact`
- `cargo test -p poise-engine reconciler::tests::flattening_reentry_confirm_recovery_is_boundary_specific -- --exact`
- `cargo test -p poise-server projector::tests::projector_shows_flatten_trigger_and_recover_policy -- --exact`
...
- `flatten` 的配置同时包含 `trigger` 和 `recover`
- `flatten.trigger` 同时支持 `"immediate"` 和 `{ flatten_confirm = { bps } }`
- `flatten.recover` 同时支持 `"back_in_band"` 和 `{ reentry_confirm = { bps } }`
...
- `flatten.trigger = "immediate"` 不经过 `FlattenPending`
