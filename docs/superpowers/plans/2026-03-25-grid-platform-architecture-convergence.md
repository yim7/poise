...
**Architecture:** 保留 `core` 的纯策略/风控和 `engine` 的状态机骨架，但把当前散落在 `server` 内的 mutate/persist/publish 流程收敛到单一应用服务。身份模型不退化成裸 `symbol`，而是引入名义化 `GridId`，当前值仍由 `symbol` 派生；对外协议下沉到独立 `grid-protocol` crate，`storage` 通过单一状态仓储接口原子保存快照和领域事件。同时把几个已经确认有语义偏差的名字一并修正：外部一等对象统一为 `Grid`，无语义容器名改成职责名，单位信息直接进字段名，订单和价格模型与真实数据来源保持一致。详见[平台架构 spec](../specs/2026-03-24-grid-platform-architecture-design.md) 和[策略研究](../../grid-strategy-product-theory-research.md)。
...
- `Cargo.toml`：把 `grid-protocol` 加入 workspace members
...
- `server/src/config.rs`：去掉 `instance_id()` 语义，改为 `grid_id()`
...
- `tui/src/api_client.rs`：改用 `grid-protocol`
...
- `tui/src/protocol.rs`：改成对 `grid-protocol` 的 re-export，并保留 fixture 测试
...
- `grid_id()` 由 `symbol` 派生，重复 `symbol` 仍被拒绝
...
```bash
cargo test -p grid-core strategy::tests::
cargo test -p grid-engine manager::tests::add_grid_rejects_duplicate_grid_ids
cargo test -p grid-server config::tests::parses_config_file_with_instances_and_exchange assembly::tests::assemble_rejects_duplicate_symbols
```
...
同时完成：
- `server::config` 里的 `InstanceConfig` 改成 `GridDefinition`，`instance_id()` 改成 `grid_id()`
...
```bash
cargo test -p grid-storage sqlite::tests::save_transition_persists_snapshot_and_events_atomically
cargo test -p grid-server runtime::tests::runtime_start_fails_when_buffered_user_data_replay_cannot_be_persisted
```
...
- `domain_events` 记录要带 `grid_id`、序列化后的事件和时间戳
...
cargo test -p grid-server http::tests:: runtime::tests::
```
...
```bash
cargo test -p grid-server http::tests:: runtime::tests:: tests::startup_flow_serves_instances_and_snapshots
```
...
要求：
- `grid-server` 文件结构包含 `application.rs` 和 `runtime.rs`
...
```bash
cargo test -p grid-engine reconciler::tests::
```
...
### Task 5: 抽出 `grid-protocol`，让 server 和 TUI 共用外部契约并完成对外 `Grid` 命名

**Files:**
- Create: `grid-protocol/Cargo.toml`
- Create: `grid-protocol/src/lib.rs`
- Create: `grid-protocol/src/grid.rs`
- Create: `grid-protocol/src/event.rs`
- Modify: `Cargo.toml`
- Modify: `server/src/http.rs`
- Modify: `server/src/websocket.rs`
...
```bash
cargo test -p grid-server http::tests:: websocket::tests::
cargo test -p grid-tui api_client::tests:: protocol::tests::
```

Expected: 旧测试在协议模块迁移前失败或无法编译。

- [x] **Step 3: 在 `grid-protocol` 里定义客户端 DTO，而不是复用内部 engine/core 类型**
...
要求：
- `server/src/http.rs` / `websocket.rs` 只输出 `grid-protocol` DTO
- `tui` 删除本地协议重复定义
...
```bash
git add Cargo.toml grid-protocol server/src/http.rs server/src/websocket.rs tui/src/api_client.rs tui/src/app.rs tui/src/main.rs tui/src/views/dashboard.rs tui/src/views/instance.rs
