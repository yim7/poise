...
创建 `application/src/session_effect_queue.rs`，先写测试和类型骨架：
...
mod tests {
    ...
    use super::{
        ...
        SessionEffectQueue, SessionPendingEffectState, SessionQueueAction, SessionTrackEffect,
        WakeSignal,
    };
    ...
    fn deferred_effect_blocks_only_its_track_until_matching_wake() {
        let queue = SessionEffectQueue::default();
        queue.enqueue_batch(vec![effect_for_track("btc-core", "btc-effect-1", "btc-batch", 1)]);
        queue.enqueue_batch(vec![effect_for_track("eth-core", "eth-effect-1", "eth-batch", 1)]);
    ...
    fn queue_snapshot_exposes_display_dto_without_batch_ordering() {
        ...
        assert_eq!(snapshot.pending_effects[0].state, SessionPendingEffectState::Queued);
    }
...
- `enqueue_batch(...)` 按 `track_id` 分组，把同一 batch 内 effect 按 `sequence` 排序，并把未暂停的 track 放入 `ready_tracks`。
...
- `CancelReceiptResolution::Unknown` 创建 `FollowUpRetirementToken`，把 token 映射到当前 cancel effect 的内部 batch 指针，并返回 `CancelQueueAction::AwaitingFollowUpRetirement { token, .. }`。
- `record_follow_up_retirement(...)` 只接收 `FollowUpRetirementResolution { token, closed_order_id }`，由 queue 内部根据 token 找到 downstream submit 并返回 action。调用方不能构造或读取 `batch_id` / `sequence`。
...
`SessionTrackEffect::from_transition_effects(...)` 在 `session_effect_queue.rs` 中实现：
...
async fn worker_dispatches_current_session_queue_effect() {
    ...
    queue.enqueue_batch(vec![SessionTrackEffect {
...
- writeback outcome unknown 仍返回 Err，让外层触发 blocked 并记录日志。
...
- `Unknown` 通过 queue 返回 `AwaitingFollowUpRetirement { reason, token }`，并触发 bounded open-order sync；sync 完成后用 token 回到 `record_follow_up_retirement(...)`。
...
在 bounded open-order sync 完成后的 follow-up 路径中，不再查询 downstream submit 列表，也不构造 `FollowUpRetirementRequest`。该路径只把 queue 之前返回的 `FollowUpRetirementToken` 和交易所确认的 closed order id 提交给 `record_follow_up_retirement(...)`；真正“哪些 downstream submit 需要废弃”的判断由 queue 内部完成。
...
async fn fresh_session_clears_session_queue_without_store_cleanup() {
    let queue = SessionEffectQueue::default();
    queue.enqueue_batch(vec![submit_effect("submit-1", "old-session-batch", 0)]);
...
```bash
cargo test -p poise-application follow_up_retirement_token_is_resolved_by_queue fresh_session_clears_session_queue_without_store_cleanup -- --nocapture
```
...
`record_follow_up_retirement(...)` 是单一领域动作。它只接收 queue 之前生成的不透明 `FollowUpRetirementToken`，在 queue 内部找到同 track、同 batch 的 downstream submit，返回 `FollowUpQueueAction::SupersededDownstream` 并把这些 submit 从 queue 中移除。调用方只处理 action，不保存 request 生命周期，也不直接读取或构造 `batch_id + sequence`。
...
bounded open-order sync 确认对应订单已不在交易所 open orders 后，只回传 token 和业务订单身份：

```rust
let action = self
    .session_effect_queue
    .record_follow_up_retirement(FollowUpRetirementResolution {
        token,
        closed_order_id,
    });
