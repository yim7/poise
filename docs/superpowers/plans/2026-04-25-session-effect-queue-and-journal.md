...
- cancel follow-up 使用两阶段 resolution plan，而不是让调用方重复解释 snapshot。bounded open-order sync 后先调用 `plan_cancel_follow_ups_from_open_order_snapshot(...)` 得到 `CancelFollowUpResolutionPlan`；application 只读取 `requires_reconcile()` 决定本次 durable sync 是否需要升级为 reconcile。durable commit 成功后，再调用 `commit_cancel_follow_up_resolution(plan)` 修改 queue 并返回 action。
...
`CommittedTrackWrite` 不再返回 `effects`。本次 transition 的 `effects` 由 `MutationExecutor` 从内存里的 `TrackTransition` 交给 `SessionEffectQueue` enqueue；queue 返回 queue-owned journal projection，再由应用层 journal adapter 转成独立的 `EffectJournalEntry` 写入 journal。
...
`EffectJournalEntry` 放在 journal / persistence 边界，不放在 `session_effect_queue.rs`。它是诊断输入 DTO；从 queue-owned journal projection 到 journal entry 的转换由应用层本地完成，journal trait 不直接依赖 `SessionTrackEffect`，`EffectJournalEntry` 也不反向知道 session queue 的运行类型。
...
`enqueue_transition_effects(...)` 在 `session_effect_queue.rs` 中实现。调用方不提供 `batch_id` / `sequence`；queue 内部生成当前 session 的 batch/effect identity，并返回 `EnqueuedTransitionEffects`。调用方只能读取 effect ids 和 queue-owned journal projection，不能把 worker 执行 DTO 当成 journal adapter 输入：
...
`EnqueuedTransitionEffects` 只在 crate 内输出 queue-owned journal projection；应用层 journal adapter 再把 projection 转成 `EffectJournalEntry`，避免 queue 直接依赖持久 DTO，也避免 crate 外部依赖 batch/sequence：
...
```bash
cargo test -p poise-server worker_dispatches_current_session_queue_effect -- --nocapture
cargo test -p poise-server worker_does_not_dispatch_persisted_effects_from_previous_session -- --nocapture
```
...
- `Unknown` 通过 queue 返回 `AwaitingCancelFollowUp { reason }`，并触发 bounded open-order sync；sync 完成后先由 `plan_cancel_follow_ups_from_open_order_snapshot(...)` 根据完整 `CompleteOpenOrderSnapshot` 生成 resolution plan，调用方只读取 `requires_reconcile()` 决定本次 durable sync 是否升级为 reconcile；commit 成功后再用 `commit_cancel_follow_up_resolution(plan)` 修改 queue。订单已不在 open orders 时返回 `FollowUpQueueAction::Closed`，标记原 cancel effect 成功、废弃 downstream submit，并且即使 downstream 为空也触发 reconcile；订单仍在 open orders 时消费本次 follow-up，把原 cancel effect 转回 queued，暂停到下一次 `WakeSignal::ExchangeState` 后重试，并跳过本次 sync 末尾的同源 exchange-state wake。
...
cargo test -p poise-server worker_dispatches_current_session_queue_effect -- --nocapture
cargo test -p poise-server worker_does_not_dispatch_persisted_effects_from_previous_session -- --nocapture
cargo test -p poise-server deferred_effect_wakes_after_fresh_market_observation -- --nocapture
cargo test -p poise-server deferred_effect_wakes_after_exchange_reconcile -- --nocapture
cargo test -p poise-server deferred_effect_ignores_wrong_wake_signal -- --nocapture
```
...
在 bounded open-order sync 完成后的 follow-up 路径中，不再查询 downstream submit 列表，也不构造 `FollowUpRetirementRequest`。该路径先把完整 `CompleteOpenOrderSnapshot` 提交给 `plan_cancel_follow_ups_from_open_order_snapshot(...)` 得到 opaque plan；application 根据 plan 决定本次 sync 是否升级为 reconcile，durable commit 成功后再把同一个 plan 交给 `commit_cancel_follow_up_resolution(plan)`。queue 内部根据之前保存的 follow-up 指针判断 unknown cancel 对应订单是仍在 open orders 还是已经关闭。
...
```bash
cargo test -p poise-application mutation_executor::tests:: -- --nocapture
cargo test -p poise-application session_effect_queue -- --nocapture
```
...
fn cancel_follow_up_is_resolved_from_complete_open_order_snapshot() {
    ...
    assert_eq!(queue.claim_next().unwrap().effect_id, effect_ids[0]);
    let action = queue.record_cancel_resolution(
        &effect_ids[0],
        CancelReceiptResolution::Unknown {
    ...
    assert_eq!(
        ...
        vec![FollowUpQueueAction::Closed {
            cancel_effect_id: effect_ids[0].clone(),
...
```bash
cargo test -p poise-application cancel_follow_up_is_resolved_from_complete_open_order_snapshot -- --nocapture
cargo test -p poise-application fresh_session_clears_session_queue_without_store_cleanup -- --nocapture
```
...
cancel follow-up 是一个 resolution plan，而不是“先读一遍 snapshot、再用另一个方法重新解释一遍 snapshot”。bounded open-order sync 完成后，调用方只提交完整 `CompleteOpenOrderSnapshot` 给 `plan_cancel_follow_ups_from_open_order_snapshot(...)`；queue 生成 opaque plan 并隐藏 token / downstream 解释细节。application 只读取 `requires_reconcile()`，用于在 durable commit 前决定是否把本次 sync 升级为 reconcile。commit 成功后，调用方把同一个 plan 交回 `commit_cancel_follow_up_resolution(plan)`；queue 先整体校验 plan 仍匹配当前运行状态，再整体修改运行状态并返回 `FollowUpQueueAction`，不能出现前半个 resolution 已提交、后半个 resolution 才发现 stale 的半状态。订单已不在 open orders 时，返回 `FollowUpQueueAction::Closed`，其中包含原 cancel effect id、被废弃的 downstream submit id，以及无论 downstream 是否为空都必须触发 reconcile 的事实。订单仍在 open orders 时，queue 消费 follow-up 指针，把原 cancel effect 转回 queued，暂停到下一次 `WakeSignal::ExchangeState`，并且不释放 downstream submit；本次 sync 的 exchange-state wake 已经被这个 still-open 结果消费，不能立刻再次唤醒同一个 track。调用方不保存 token 生命周期，也不直接读取或构造 `batch_id + sequence`。
...
let plan = self
    .session_effect_queue
    .plan_cancel_follow_ups_from_open_order_snapshot(&TrackId::new(id), &open_orders);
let effective_mode = if plan.requires_reconcile() {
    ExchangeSyncMode::RecoverAndReconcile
} else {
    mode
};
// durable sync + commit succeeds here
let actions = self
    .session_effect_queue
    .commit_cancel_follow_up_resolution(plan);
for action in actions {
    self.handle_follow_up_queue_action(id, action).await?;
}
```

删除 `retry_pending_follow_up_retirements_best_effort`。cancel follow-up 不再有 pending retry 概念；空 plan 可以没有 action；非空 plan 在 commit 时如果没有匹配到当前 session queue 中的 follow-up 指针，或者 plan 内存在重复 / 自相冲突的 cancel follow-up，queue 返回 `Blocked` 且不做部分提交，不能静默当成 no-op。

如果当前代码没有独立 `handle_follow_up_queue_action(...)`，本 task 新增一个私有 helper。它只根据 `FollowUpQueueAction` 做两类事：

- `Closed`：best-effort 写 cancel succeeded 和 downstream superseded journal outcomes，并触发 fresh reconcile。
- `StillOpen`：记录诊断并跳过本次 sync 末尾的 `WakeSignal::ExchangeState`；queue 已经把原 cancel effect 放回 queued 并暂停到下一次 exchange-state wake。
- `Blocked`：返回错误或记录明确异常；不释放任何 submit。
...
- 验收：`cargo test -p poise-server runtime::startup_bootstrap::tests::complete_startup_cancels_inherited_orders_and_rebuilds_fresh_executor_state -- --nocapture`
- 说明：旧 storage pending submit query 测试只用于暴露旧 outbox 行为；Task 5 删除该查询后，不再作为当前验收入口。
...
```bash
cargo test -p poise-storage recent_track_effects -- --nocapture
cargo test -p poise-storage effect_journal -- --nocapture
cargo check -p poise-application -p poise-storage -p poise-server
```
...
```bash
cargo test -p poise-server projector_labels_effects_as_history_not_current_execution_queue -- --nocapture
cargo test -p poise-server projector_renders_current_session_queue_from_snapshot -- --nocapture
```
...
cargo test -p poise-application mutation_executor::tests:: -- --nocapture
cargo test -p poise-application runtime_lifecycle_service::tests:: -- --nocapture
cargo test -p poise-server effect_worker -- --nocapture
cargo test -p poise-server projector::tests:: -- --nocapture
cargo test -p poise-storage recent_track_effects -- --nocapture
cargo test -p poise-storage effect_journal -- --nocapture
cargo check -p poise-application -p poise-storage -p poise-server
