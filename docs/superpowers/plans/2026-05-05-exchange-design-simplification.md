...
**Architecture:** 保留现有 `ExecutionPort` / `MarketDataPort` / `AccountPort` / `AccountSummaryPort` / `MetadataPort` 这组有效边界；新增一个轻量 `ExchangePorts` bundle 删除重复的 `Connected` 容器。只把已经跨交易所重复的组合和启动资金探测策略收进共享边界，REST/WS 请求签名、字段映射、账户模式等交易所细节继续留在各 exchange crate 内。
...
- `engine/src/ports.rs`：定义系统消费交易所能力的 port traits；新增 `ExchangePorts` 只负责组合现有 ports。
- `server/src/exchange.rs`：保留 server 侧 `Exchange { venue, ports }` 包装，负责带上 venue 和暴露 runtime 所需 ports。
- `exchanges/*/src/connected.rs`：只负责把交易所私有 REST/WS client 组装成 `ExchangePorts`；逐步删除重复的 `Connected` 容器和纯透传 wrapper。
- `server/src/assembly.rs`：只按配置选择连接哪个交易所，不再手动拆 `connected.execution()` 等字段。
- `server/src/runtime/startup_bootstrap.rs`：启动时只调用统一的启动资金探测 port，不再按 venue 或 mode 分支。
...
在每个 exchange 的 connected tests 中增加一个结构性断言或 compile-only use：`connect()` 仍能构造全部 ports，但不依赖本地 `*MarketData`、`*AccountSummary`、`*Metadata` 这类纯透传 wrapper。
...
### Task 4: 把启动资金探测策略移到 exchange ports，删除 server venue 分支
...
- Modify: `server/src/assembly.rs`
- Modify: `exchanges/*/src/connected.rs`
- Test: `server/src/runtime/startup_bootstrap.rs`
- Test: `server/src/assembly.rs`

- [ ] **Step 1: 写失败测试**

在 `server/src/assembly.rs` 修改测试：新增断言 `build_startup_capacity_mode` 不再存在或不再按 `Venue::Bybit | Venue::Hyperliquid | Venue::Okx` 分支。

在 `server/src/runtime/startup_bootstrap.rs` 增加测试：

```rust
#[tokio::test]
async fn startup_capacity_is_probed_through_exchange_port_with_track_leverage() {
    // FakeStartupCapacityPort 记录 instrument 和 startup_leverage
...
cargo test -p poise-server startup_bootstrap::tests::startup_capacity_is_probed_through_exchange_port_with_track_leverage
```
...
```rust
#[async_trait]
pub trait StartupCapacityPort: Send + Sync {
    async fn get_startup_capacity_snapshot(
        &self,
...
把它加入 `ExchangePorts`。删除 `RuntimeStartupCapacityMode` 和 `RuntimeStartupDefinition.capacity_mode`，`startup_bootstrap` 统一调用 `runtime.startup_capacity.get_startup_capacity_snapshot(...)`。
...
```bash
git add engine server exchanges docs/superpowers/plans/2026-05-05-exchange-design-simplification.md
git commit -m "refactor: move startup capacity probing to exchange ports"
```
...
```bash
rg -n "ExchangePorts|StartupCapacityPort|新增交易所" docs README.md
git diff --check
```
