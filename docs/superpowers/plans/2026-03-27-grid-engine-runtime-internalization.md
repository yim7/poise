...
- `docs/superpowers/specs/2026-03-27-grid-engine-runtime-internalization-design.md`：实现中如果接口命名微调，同步 spec
- `docs/superpowers/plans/2026-03-27-grid-engine-runtime-internalization.md`：执行过程中同步任务清单
...
```bash
cargo test -p grid-engine reconciler::tests::reconcile_reads_budget_from_runtime -- --exact
cargo test -p grid-engine manager::tests::resume_grid_recomputes_status_without_external_budget_store -- --exact
```
...
cargo test -p grid-server assembly::tests::
```

Expected: `grid-engine` 相关测试通过，`grid-server` 组装测试保持全绿。
...
```bash
cargo test -p grid-engine manager::tests::exchange_state_observation_clears_stale_pending_order_when_anchor_is_not_preserved -- --exact
cargo test -p grid-engine manager::tests::exchange_state_observation_preserves_submit_anchor_when_requested -- --exact
cargo test -p grid-server startup_sync_uses_exchange_state_observation_to_restore_live_state -- --exact
```
...
```bash
cargo test -p grid-engine manager::tests::exchange_state_observation_ -- --nocapture
cargo test -p grid-server startup_sync_ -- --nocapture
...
```bash
cargo test -p grid-engine manager::tests::observe_position_with_cached_reference_price_reconciles_immediately -- --exact
cargo test -p grid-engine manager::tests::observe_canceled_order_with_cached_reference_price_reconciles_immediately -- --exact
cargo test -p grid-engine manager::tests::observe_filled_order_does_not_reconcile_before_position_update -- --exact
cargo test -p grid-server runtime::tests::position_update_reconciles_without_runtime_follow_up_command -- --exact
```
...
```bash
cargo test -p grid-engine manager::tests::
cargo test -p grid-server runtime::tests::position_update_
cargo test -p grid-server runtime::tests::terminal_order_update_
```
...
```bash
cargo test -p grid-engine runtime::tests::pending_order_ -- --nocapture
cargo test -p grid-server runtime::tests::effect_worker_ -- --nocapture
...
- Modify: `docs/superpowers/plans/2026-03-27-grid-engine-runtime-internalization.md`
...
Expected: `grid-engine`、`grid-server` 全绿；`cargo test --workspace` 由人工最终统一执行。
...
```bash
git add docs/superpowers/specs/2026-03-27-grid-engine-runtime-internalization-design.md docs/superpowers/plans/2026-03-27-grid-engine-runtime-internalization.md
