...
fn deserializes_track_diagnostics_response() {
    ...
    assert_eq!(payload.items[0].message, "desired exposure -3.9534 -> -3.7500");
}
```
...
Expected: FAIL，当前 activity 仍包含 `desired exposure ...`
...
Expected: PASS，并更新断言为 activity 不再包含 `desired exposure ...`
...
- `cargo test -p poise-server http::tests::get_track_diagnostics_returns_exposure_target_changed_events -- --exact` → 初次失败，`left: 404 right: 200`；补齐路由后再次失败，断言缺少 `desired exposure`；补齐测试夹具后通过，`1 passed`
...
- diagnostics 面板包含 `desired exposure ...`
