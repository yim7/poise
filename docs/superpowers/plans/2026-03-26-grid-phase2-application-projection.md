...
- `server/src/http.rs`：从 `/tracks/:id/snapshot` 切换到 `/tracks/:id`，并改 typed command
...
```bash
cargo test -p poise-tui protocol::tests::deserializes_grid_list_response -- --exact
cargo test -p poise-tui protocol::tests::deserializes_grid_detail_view -- --exact
...
- [x] **Step 4: 跑 `poise-tui` 协议测试，确认新 DTO 固定下来**

Run:

```bash
cargo test -p poise-tui protocol::tests::
```
...
#[tokio::test]
async fn list_recent_grid_effects_filters_by_track_id_and_limit() {
    let storage = SqliteStorage::in_memory().unwrap();
...
```bash
cargo test -p poise-storage sqlite::tests::list_recent_grid_events_returns_timestamped_records_in_order -- --exact
cargo test -p poise-storage sqlite::tests::list_recent_grid_effects_filters_by_track_id_and_limit -- --exact
```
...
pub trait GridReadRepositoryPort: Send + Sync {
    ...
    async fn load_grid_snapshot(&self, track_id: &GridId) -> Result<Option<GridRuntimeSnapshot>>;
...
- [x] **Step 4: 跑 `poise-storage` 定向和全量测试**
...
Expected: 新增测试通过，`poise-storage` 全绿。
...
async fn list_grid_sources_reads_all_registered_snapshots() {
    ...
    assert_eq!(sources[0].snapshot.track_id.as_str(), "btc-core");
}
...
async fn load_detail_source_reads_snapshot_events_and_effects() {
    ...
    assert_eq!(source.snapshot.track_id.as_str(), "btc-core");
...
```bash
cargo test -p poise-server projector::tests::project_list_item_summarizes_execution_state -- --exact
cargo test -p poise-server query_service::tests::list_grid_sources_reads_all_registered_snapshots -- --exact
cargo test -p poise-server query_service::tests::load_detail_source_reads_snapshot_events_and_effects -- --exact
```
...
- `list_grid_sources()` 用于 `/tracks` 列表查询
...
- [x] **Step 4: 跑 `poise-server` 查询 / 投影定向测试**

Run:

```bash
cargo test -p poise-server projector::tests::
cargo test -p poise-server query_service::tests::
```
...
```bash
cargo test -p poise-server write_service::tests::command_persists_transition_and_emits_grid_write_committed -- --exact
cargo test -p poise-server runtime::tests::startup_sync_preserves_submitting_pending_order_until_exchange_catches_up -- --exact
```
...
- `grid_instruments()`、`resolve_track_id()` 暂时保留给 `runtime` 做内部路由，不暴露给 HTTP
...
- [x] **Step 4: 跑 `poise-server` 写侧和运行时回归测试**
...
cargo test -p poise-server runtime::tests::startup_sync_preserves_submitting_pending_order_until_exchange_catches_up -- --exact
cargo test -p poise-server runtime::tests::effect_worker_does_not_resubmit_when_matching_pending_order_is_already_restored -- --exact
```
...
- [x] **Step 1: 先写失败测试，锁住新路由和新 WS payload**
...
- [x] **Step 2: 运行定向测试，确认 transport 仍在使用旧 contract**

Run:

```bash
cargo test -p poise-server http::tests::list_grids_returns_grid_list_response -- --exact
cargo test -p poise-server websocket::tests::broadcasts_grid_detail_changed_after_write_commit -- --exact
```

实现前预期：

- `/tracks` 仍返回 `Vec<GridSummary>`
- `/tracks/:id/snapshot` 仍存在
- WS 仍广播旧 `WsEvent`

- [x] **Step 3: 最小实现新的 HTTP / WS 适配**
...
- `POST /tracks/:id/commands` 使用 `GridCommandRequest { command: GridCommandType }`
...
```rust
match notification {
    GridInternalNotification::GridWriteCommitted { track_id }
    | GridInternalNotification::GridEffectStateChanged { track_id } => {
        let item = projector.project_list_item(&query.load_list_source(&track_id).await?);
        let detail = projector.project_detail(&query.load_detail_source(&track_id).await?);
...
- [x] **Step 4: 跑 `poise-server` transport 定向测试**

Run:

```bash
cargo test -p poise-server http::tests::
cargo test -p poise-server websocket::tests::
cargo test -p poise-server tests::startup_flow_serves_grid_list_and_detail -- --exact
```
...
- `GET /tracks` 已返回 `GridListResponse`
- `GET /tracks/:id` 已替代旧的 `/tracks/:id/snapshot`
- WebSocket 已只推 `GridStreamEvent`
...
- [x] **Step 2: 运行定向测试，确认 TUI 仍绑定旧 snapshot 模型**

Run:

```bash
cargo test -p poise-tui api_client::tests::list_grids_decodes_grid_list_response -- --exact
cargo test -p poise-tui app::tests::apply_grid_detail_updates_current_detail_without_snapshot_cache -- --exact
cargo test -p poise-tui tests::load_initial_state_fetches_list_then_selected_detail -- --exact
```

Expected: 当前 `ApiClient`、`App`、`main.rs` 还在使用 `GridSummary + GridSnapshot + WsEvent`，测试失败。

- [x] **Step 3: 最小实现新的 client / app / runtime loop**
...
- `load_initial_state()` 改为：
  - 先拉 `/tracks`
  - 再拉当前选中 grid 的 `/tracks/:id`
- `handle_ws_event()` 直接应用 `GridListItemChanged` / `GridDetailChanged`

- [x] **Step 4: 跑 `poise-tui` 传输与状态测试**

Run:

```bash
cargo test -p poise-tui api_client::tests::
cargo test -p poise-tui app::tests::
cargo test -p poise-tui tests::load_initial_state_fetches_list_then_selected_detail -- --exact
```
...
- [x] **Step 2: 运行定向测试，确认视图仍在渲染旧 snapshot / event**

Run:

```bash
cargo test -p poise-tui views::dashboard::tests::renders_dashboard_rows_from_grid_list_items -- --exact
cargo test -p poise-tui views::instance::tests::renders_grid_detail_execution_activity_and_commands -- --exact
```

Expected: 旧视图仍读取 `GridSnapshot`、`pending_order` 和 `WsEvent`，测试失败。

- [x] **Step 3: 最小实现视图切换和文档更新**
...
- [x] `GridPlatformService` 已删除，`server` 顶层边界变成 `write/query/projector/notifications`
- [x] `protocol` 不再暴露 `GridSnapshot`、原始 `PendingOrder`、原始 `DomainEvent`
- [x] `storage` 提供只读查询端口，query 侧不再复用写侧事务接口
- [x] `GET /tracks` 返回 `GridListResponse`
- [x] `GET /tracks/:id` 返回 `GridDetailView`
- [x] `POST /tracks/:id/commands` 使用 typed command
- [x] WebSocket 只推 `GridListItemChanged` / `GridDetailChanged`
- [x] TUI 启动流程改为“拉列表 + 拉当前详情”
- [x] TUI 视图只依赖新读模型
- [x] `cargo test -p poise-server` 通过
- [x] `cargo test -p poise-tui` 通过
- [x] `cargo test` 通过
