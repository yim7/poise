...
- [x] **Step 4: 跑 `grid-server` transport 定向测试**
...
- `GET /grids` 已返回 `GridListResponse`
- `GET /grids/:id` 已替代旧的 `/grids/:id/snapshot`
...
- [x] `GET /grids` 返回 `GridListResponse`
- [x] `GET /grids/:id` 返回 `GridDetailView`
- [x] `POST /grids/:id/commands` 使用 typed command
- [x] WebSocket 只推 `GridListItemChanged` / `GridDetailChanged`
- [x] TUI 启动流程改为“拉列表 + 拉当前详情”
- [x] TUI 视图只依赖新读模型
- [x] `cargo test -p grid-server` 通过
- [x] `cargo test -p grid-tui` 通过
- [x] `cargo test` 通过
