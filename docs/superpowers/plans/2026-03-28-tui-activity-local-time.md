...
**Goal:** 让 `poise-tui` 的 `Activity` 列表按本机时区显示时间戳，同时保持协议和服务端输出不变。
...
Run: `cargo test -p poise-tui views::instance::tests::renders_activity_timestamp_in_local_time -- --exact`
...
Run: `cargo test -p poise-tui activity_timestamp -- --nocapture`
Expected:
...
- 状态：已完成
- 验收：
  - `cargo test -p poise-tui views::instance::tests::renders_activity_timestamp_in_local_time -- --exact`
  - `cargo test -p poise-tui activity_timestamp -- --nocapture`
