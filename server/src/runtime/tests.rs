...
async fn runtime_fixture_with_options(
    ...
    let account_monitor =
        build_test_account_monitor(exchange.clone() as Arc<dyn ExchangePort>, events.clone()).await;
...
impl FakeExchange {
    ...
    fn with_submit_error(position: Position, open_orders: Vec<ExchangeOrder>, error: &str) -> Self {
